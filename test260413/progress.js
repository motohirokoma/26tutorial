/**
 * Claude Code Tutorial - Progress Tracking (ES Module)
 *
 * Stores chapter completion + streak in localStorage for instant UI,
 * and syncs to Firestore (users/{uid}) when the user is signed in.
...
const STREAK_KEY = 'claude-tutorial-streak';

const CHAPTERS = [
  { id: '01', title: 'Introduction', hours: 1, phase: 'basics' },
  { id: '02', title: 'Installation', hours: 1, phase: 'basics' },
  { id: '03', title: 'Basic Commands', hours: 1.5, phase: 'basics' },
  { id: '04', title: 'Prompt Design', hours: 1.5, phase: 'basics' },
  { id: '05', title: 'File Operations', hours: 1.5, phase: 'core' },
  { id: '06', title: 'Git Integration', hours: 1.5, phase: 'core' },
  { id: '07-1', title: 'GitHub: Issues & PR', hours: 1, phase: 'core' },
  { id: '07-2', title: 'GitHub: Actions & Team', hours: 1, phase: 'core' },
  { id: '08-1', title: 'Debugging', hours: 1, phase: 'core' },
  { id: '08-2', title: 'Testing & TDD', hours: 1, phase: 'core' },
  { id: '09', title: 'CLAUDE.md', hours: 1.5, phase: 'advanced' },
  { id: '10', title: 'Skills & Hooks', hours: 1.5, phase: 'advanced' },
  { id: '11', title: 'MCP & Agents', hours: 1, phase: 'advanced' },
  { id: '12-1', title: 'Website: Setup', hours: 1.5, phase: 'projects' },
  { id: '12-2', title: 'Website: Deploy', hours: 1.5, phase: 'projects' },
  { id: '13-1', title: 'App: Backend', hours: 1.5, phase: 'projects' },
  { id: '13-2', title: 'App: Frontend', hours: 1.5, phase: 'projects' },
  { id: '14-1', title: 'iOS: Data & API', hours: 1.5, phase: 'projects' },
  { id: '14-2', title: 'iOS: UI & Summary', hours: 1.5, phase: 'projects' },
];

const TOTAL_HOURS = 26;

const CHAPTER_FILES = {
  '01': '01-introduction.html',
  '02': '02-installation.html',
  '03': '03-basic-commands.html',
  '04': '04-prompt-design.html',
  '05': '05-file-operations.html',
  '06': '06-git-integration.html',
  '07-1': '07-1-github-workflow.html',
  '07-2': '07-2-github-workflow.html',
  '08-1': '08-1-debugging.html',
  '08-2': '08-2-testing.html',
  '09': '09-claude-md-settings.html',
  '10': '10-skills-hooks.html',
  '11': '11-mcp-agents.html',
  '12-1': '12-1-project-website.html',
  '12-2': '12-2-project-website.html',
  '13-1': '13-1-project-app.html',
  '13-2': '13-2-project-app.html',
  '14-1': '14-1-project-ios.html',
  '14-2': '14-2-project-ios.html',
};

let currentUser = null;
let cloudUnsub = null;
// When true, the next snapshot/write should NOT trigger a push (we're applying remote).
let applyingRemote = false;
// True only for the first snapshot after sign-in (to merge existing local state with cloud).
...
function mergeProgress(local, cloud) {
  // Union of keys; keep the earliest completion date when both exist.
  const merged = { ...cloud };
...
function mergeStreak(local, cloud) {
  ...
  // Prefer the one with the more recent lastDate; take the larger count if dates match.
...
function subscribeToCloud(uid) {
  ...
  cloudUnsub = onSnapshot(userDocRef(uid), (snap) => {
    // Skip echoes of our own unconfirmed writes — local state is already correct.
    if (snap.metadata && snap.metadata.hasPendingWrites) return;

    const cloud = snap.exists() ? snap.data() : null;

    if (firstSnapshotForUser) {
      // Mark done FIRST so any concurrent snapshot falls through to the else branch.
      firstSnapshotForUser = false;

      // Initial sync: union-merge local and cloud so no existing progress is lost.
      ...
      // If local had extras (or doc didn't exist), push merged state up.
      ...
      // Subsequent committed snapshots: trust cloud as source of truth (supports deletions).
      applyingRemote = true;
...
function getPhaseStats() {
  const data = getProgress();
  const phases = { basics: { done: 0, total: 0 }, core: { done: 0, total: 0 }, advanced: { done: 0, total: 0 }, projects: { done: 0, total: 0 } };
  CHAPTERS.forEach(ch => {
...
function updateAllUI() {
  ...
  const phaseMap = { 'pct-basics': 'basics', 'pct-core': 'core', 'pct-adv': 'advanced', 'pct-proj': 'projects' };
  for (const elId in phaseMap) {
    const el = document.getElementById(elId);
    if (el) {
      const pct = phaseStats[phaseMap[elId]];
      el.textContent = pct + '%';
